...
db.instagramers.aggregate(
	...
	{ $project: {_id:0,"day":{"$concat": [ { "$substr": ["$_id",0,2] }, "-", "06" ]} , "photography": "$photography" }},
	{ "$sort": { "photography": -1}}
)
...
db.instagramers.aggregate(
	...
	{ $project: {_id:0,"nationality":"$_id", "photography": "$photography" }},
	{ "$sort": { "photography": -1}}
	)	
	
	
	
//Sortir les lieux de type "food" ou "cafe" ou "restaurant" ou "meal_delivery"  les mieux notés de Paris (d'après la notation Google). On considère la note à partir d'au moins 100 photos publiés durant le mois de Juin 2014.
db.instagramers.aggregate(
	{$unwind: "$photos"},
	...
	{$match: {"count":{ $gt: 100 }}},
	{ $project: {_id:0,"place":"$_id", "moy_avg": "$moy_avg", "count": "$count" }},
	{ "$sort": { "moy_avg": -1}}
	)
...
db.instagramers.aggregate(
	...
	{ $project: {_id:0,"nationality":"$_id", "moy_pays": "$moy_pays","nbOfPeople": "$count" }},
	{ "$sort": { "moy_pays": -1}}
	)
...
db.instagramers.aggregate(
	...
	{$project: {_id:0,"nationality":"$_id", "avg_use": "$avg_use","count":"$count"}},
	{"$sort": { "avg_use": -1}}
	)
...
db.instagramers.aggregate(
	...
	{ $project : {"_id":0,"photography": "$photography", day: {"$cond" : [{"$eq" : [ 1,"$_id"]},"Dimanche",
          {"$cond" : [{"$eq" : [2,"$_id"]},"Lundi",
          {"$cond" : [{"$eq" : [3,"$_id"]},"Mardi",
...
db.instagramers.aggregate(
	...
	{ $project : {"_id":0,"photography": "$photography","hour":{"$concat": [ { "$substr": ["$_id.hour",0,2] }, "", "H" ]},
...
//---------------------------------------------------------------------------------------------------//
// Ayant une zone de localisation, et ayant un type de cette zone (tous les deux définit par l'analyste) faire sortir les utilisateurs les plus fréquent dans cette zone.
...
db.instagramers.aggregate(
	...
	{ $group: {_id: "$nationality", "photography": { $sum: 1 } } },
	{ $project : {"_id":0,"photography": "$photography","nationality":"$_id"}},
	{ "$sort": { "photography": -1}}
)
...
db.instagramers.aggregate(
	...
	{ $project: {_id:0,"day":{"$concat": [ { "$substr": ["$_id.day",0,2] }, "/", "06" ]},"hour":{"$concat": [ { "$substr": ["$_id.hour",0,2] }, "", "H" ]}  , "photography": "$photography" }}
)
...
//https://stackoverflow.com/questions/40177205/mapreduce-function-mongodb-nodejs
